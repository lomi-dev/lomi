...
## Selected terminal shell and prompt authority (2026-09-24)

Settings selects one discovered system Bash or Zsh profile for terminal execution.
The grant captures its ID and native revision. Create defaults to that profile;
an explicit profile ID cannot select another shell. Native start and approved
attachment recheck the current definition and the one-use dispatch authority.
The shell still uses the account's configuration and permissions; cwd is not a
sandbox. Other shells and platforms remain unqualified for this MCP path.

On macOS Bash 3.2, a guarded DEBUG hook supplies the missing preexec signal while
preserving scalar/array PROMPT_COMMAND hooks and exit status. An existing user
DEBUG trap, functrace or extdebug is preserved and automatic readiness is omitted.
Explicit input keeps its existing scope, lease, ancestry and sequence checks.
After explicit input or attachment to an existing PTY, a redraw of PS1 cannot
restore readiness until a new shell completion boundary is observed. Native
human input revokes the lease. Output silence and EOF without a completion marker
remain incomplete/unknown, never inferred exit zero.

Native Bash uw9501 and Zsh Gn0P6D passed 13 checks each through stdio, Settings,
PTY and xterm, including real REPL/TUI, binary output, bounded flood/gaps and
approved reclaim. See [qualification](QUALIFICATION.md) for versions and remaining
P6 work. These tools retain the existing catalog count of 74.
...
Other engines/platforms remain unqualified. This increment brought the catalog
to 73 tools; the later upload increment brings the current total to 74.
...
Reads disclose only granted conversations, recheck authority after native work,
and identify their source as persisted checkpoints. Page one message or draft at
a time, at most8192 UTF-16 units/48KiB output, with content revisions and scalar
boundaries. Attachment bytes, credentials, raw provider metadata and system
prompts are not part of this read. Conversation IDs never authorize themselves.
Read-only MCP calls require an already initialized chat service and do not run a
provider, repair history or recover credentials. Settings metadata discovery is
an explicit user action. Create/draft/open retain existing stores and UI runtime
state; draft revisions protect user edits. Send requires a separate exact UI
decision for the selected connection/model, actual context/draft and output limit;
native request IDs deduplicate before dispatch. Stop targets the exact authorized
request. Export needs an explicit destination/data-transfer approval and preserves
source authorization until export is committed. Use the existing fixture provider
for end-to-end tests; never send a paid request for qualification. All seven tools
(list/open/read/draft/send/stop/export) and their selected-layout integration are
required before declaring the Chat module complete or pushing its milestone.

Qualification: native iyLq7U passed all 54 checks with catalog68 and five local
fixture generations, including all seven tools, mixed-layout movement/reveal,
and last-view panel/workspace/project closure. Core broker59, native Chat34,
WebKit UI14 and TypeScript passed. All affected conversations require exact read
grants; closing a final view additionally requires chat.stop. Editor guards and
draft flush precede native checkpointed cancellation; an admission barrier
...
profiles qualify Chat, Android management/layout, frames, logs, upload, download
and artifacts. Their exact evidence and limitations are in QUALIFICATION.md.
Input/output schemas are defined by the protocol crate and registered by the
helper; the contracts above and historical increments below document scopes,
identity, readiness, bounds, retry, cancellation and typed errors. Shared
requirements come from chapters 02, 03, 08, 11, 14 and audit A01–A13.

| Tool                         | Source / stage  | Contract and adapter                                                                                                                                                                                                                    | Evidence                                                                                                                       | State                                                |
| ---------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
| `lomi_operation_get`         | 03 / P1–P5      | Protocol control.rs; broker.rs / broker/operations.rs                                                                                                                                                                                   | Core68 / broker68 / protocol13 / helper2 / wire8; native 1tJWXL, KoF9rh and NHdHGE/TjYIpJ                                      | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_status`                | 03 / P1–P5      | Protocol control.rs; broker.rs / broker/operations.rs                                                                                                                                                                                   | Core68 / broker68 / protocol13 / helper2 / wire8; native 1tJWXL, KoF9rh and NHdHGE/TjYIpJ                                      | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_connect`               | 03 / P1–P5      | Protocol control.rs; broker.rs / broker/operations.rs                                                                                                                                                                                   | Core68 / broker68 / protocol13 / helper2 / wire8; native 1tJWXL, KoF9rh and NHdHGE/TjYIpJ                                      | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_workspace_list`        | 03 / P1–P5      | Protocol control.rs; broker.rs / broker/operations.rs                                                                                                                                                                                   | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_workspace_create`      | 03 / P1–P4/P5   | Protocol control.rs; broker/panels.rs, operations.rs and broker.rs; Workbench bridge                                                                                                                                                    | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_workspace_update`      | 03 / 07 / P5.3  | Closed rename/select/close DTOs; per-action scopes, exact revision and durable ancestor receipts                                                                                                                                        | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_workspace_close`       | 03 / 07 / P5.3  | Provided by lomi_workspace_update action=close; no separate alias exposed                                                                                                                                                               | Actual workspace closure via lomi_workspace_update; native wGLMu7 / ErnuOt / 0ovWOt                                            | VERIFIED alias contract; not a separate tool         |
| `lomi_panel_list`            | 03 / P1–P4/P5   | Protocol control.rs; broker/panels.rs, operations.rs and broker.rs; Workbench bridge                                                                                                                                                    | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_panel_focus`           | 03 / P1–P4/P5   | Protocol control.rs; broker/panels.rs, operations.rs and broker.rs; Workbench bridge                                                                                                                                                    | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_panel_control`         | 03 / P1–P4/P5   | Closed claim/release DTO; bounded Settings-only approval and native existing-PTY attachment                                                                                                                                             | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_panel_move`            | 03 / 07 / P5.3  | Reorder/dock/move and same-project cross-workspace tab transfer; both grants, exact resource identities                                                                                                                                 | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_panel_close`           | 03 / P1–P4/P5   | Protocol control.rs; broker/panels.rs, operations.rs and broker.rs; Workbench bridge                                                                                                                                                    | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_operation_cancel`      | 03 / P1–P5      | Protocol control.rs; broker.rs / broker/operations.rs                                                                                                                                                                                   | Core68 / broker68 / protocol13 / helper2 / wire8; native 1tJWXL, KoF9rh and NHdHGE/TjYIpJ                                      | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_events_read`           | 03 / P1–P4/P5   | Protocol control.rs; broker/panels.rs, operations.rs and broker.rs; Workbench bridge                                                                                                                                                    | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_artifact_read`         | 03 / P1–P4/P5   | Immutable PNG source classification and native authority; protocol artifact.rs, broker/artifacts.rs and private Store                                                                                                                   | Native lVi0Ip12 and 1tJWXL; immutable APK/PNG/file, source grants, quotas, migration and revoked reads                         | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_artifact_import`       | 03 / 06 / P4–P5 | Exact APK/file import with independent scopes, immutable staging, source classification and quotas                                                                                                                                      | Native lVi0Ip12 and 1tJWXL; immutable APK/PNG/file, source grants, quotas, migration and revoked reads                         | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_artifact_export`       | 03 / P5         | Source-authorized artifact to new project file; hash, parent/layout revisions, atomic native writer, durable retry                                                                                                                      | Native lVi0Ip12 and 1tJWXL; immutable APK/PNG/file, source grants, quotas, migration and revoked reads                         | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_diagnostics`           | 03 / P1–P5      | Protocol control.rs; broker.rs / broker/operations.rs                                                                                                                                                                                   | Core68 / broker68 / protocol13 / helper2 / wire8; native 1tJWXL, KoF9rh and NHdHGE/TjYIpJ                                      | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_terminal_create`       | 05 / P2         | Closed control.rs DTO; broker/terminals.rs, terminal_runs.rs, terminal_input.rs; native terminal.rs and retained terminal-runtime.ts                                                                                                    | Native Bash uw9501 / Zsh Gn0P6D: 13 checks each; core, wire and UI tests                                                       | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_terminal_run`          | 05 / P2         | Closed control.rs DTO; broker/terminals.rs, terminal_runs.rs, terminal_input.rs; native terminal.rs and retained terminal-runtime.ts                                                                                                    | Native Bash uw9501 / Zsh Gn0P6D: 13 checks each; core, wire and UI tests                                                       | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_terminal_read`         | 05 / P2         | Closed control.rs DTO; broker/terminals.rs, terminal_runs.rs, terminal_input.rs; native terminal.rs and retained terminal-runtime.ts                                                                                                    | Native Bash uw9501 / Zsh Gn0P6D: 13 checks each; core, wire and UI tests                                                       | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_terminal_input`        | 05 / P2         | Closed control.rs DTO; broker/terminals.rs, terminal_runs.rs, terminal_input.rs; native terminal.rs and retained terminal-runtime.ts                                                                                                    | Native Bash uw9501 / Zsh Gn0P6D: 13 checks each; core, wire and UI tests                                                       | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_terminal_interrupt`    | 05 / P2         | Closed control.rs DTO; broker/terminals.rs, terminal_runs.rs, terminal_input.rs; native terminal.rs and retained terminal-runtime.ts                                                                                                    | Native Bash uw9501 / Zsh Gn0P6D: 13 checks each; core, wire and UI tests                                                       | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_open`          | 04 / P2–P3/P5.7 | Closed DTO, broker/browsers.rs, atomic BrowserControl, native browser.rs and retained Workbench                                                                                                                                         | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_navigate`      | 04 / P2–P3/P5.7 | Owned generation, lease, exact origins; native commit/load                                                                                                                                                                              | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_snapshot`      | 04 / P2–P3/P5.7 | Bounded isolated-world top-level semantic DOM, private values omitted                                                                                                                                                                   | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_click`         | 04 / P2–P3/P5.7 | Snapshot-bound synthetic DOM click, durable retry                                                                                                                                                                                       | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_fill`          | 04 / P2–P3/P5.7 | Text/textarea/select/contenteditable, value length only                                                                                                                                                                                 | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_key`           | 04 / P2–P3/P5.7 | Focused top-level synthetic key events; no native default action                                                                                                                                                                        | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_scroll`        | 04 / P2–P3/P5.7 | Top-level viewport CSS deltas, snapshot/lease                                                                                                                                                                                           | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_wait`          | 04 / P2–P3/P5.7 | Bounded DOM text/element/URL/load polling                                                                                                                                                                                               | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_screenshot`    | 04 / P2–P3/P5.7 | Explicit browser.capture_composite; native WK snapshot, bounded producer, PNG and immutable artifact                                                                                                                                    | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_logs`          | 04 / P2–P3/P5.7 | browser.read; document-start main-frame errors/console/rejections, bounded rings and kind-bound cursors; native agent_dom and broker/browser_logs                                                                                       | Native 1tJWXL plus frames KjoDju13, logs a6K0Te11, profile separation Hv3cqb6 and disconnect 6im2p3                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_upload`        | 04 / P5.7       | BrowserUploadInput; exact frame/ref/source/hash and one-use Settings approval; browser.upload/read/interact; immutable <=4 MiB attachment                                                                                               | Native vuDLZZ12; deny, cancel, retry, altered source and foreign access                                                        | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_browser_download`      | 04 / P5.7       | BrowserDownloadInput; browser.download/read; exact same-origin GET, bounded native stream, private source-classified artifact and durable retry                                                                                         | Native PvUmgs12; actual page credentials, limits, redirect denial and single GET on retry                                      | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_list`          | 03/06/08, P4    | Selected managed-device metadata; no runtime startup                                                                                                                                                                                    | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_open`          | 03/06 / P4      | Selected device + workspace/panel.create/android.read; strict revision/retry; actual retained manual-start panel                                                                                                                        | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_start`         | 03/06 / P4      | Selected device/panel, android.control + read, revision/retry; ready requires guest/RPC/ADB/IME; 180s bound                                                                                                                             | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_stop`          | 03/06 / P4      | Selected device/panel/exact owned generation; android.control + read; revision/retry; confirmed exit; no implicit force                                                                                                                 | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_snapshot`      | 06 / P4         | AndroidSnapshotInput; android.observe/control/read; owned generation, bounded hierarchy; broker/android_capture.rs                                                                                                                      | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_screenshot`    | 06 / P4         | AndroidScreenshotInput/Artifact; android.capture/control/read; bounded native PNG, source authority                                                                                                                                     | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_input`         | 06 / P4         | AndroidInput; android.interact/control/read; RAM lease, sequences, native focus and human takeover                                                                                                                                      | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_install_apk`   | 06 / P4         | AndroidInstallInput; android.install/control/read + files.read; immutable APK and exact Settings approval                                                                                                                               | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_launch`        | 06 / P4         | AndroidLaunchInput; android.launch/control/read; selected packages and explicit launch receipt                                                                                                                                          | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_logcat`        | 06 / P4         | AndroidLogcatInput; android.logs/control/read; selected package, PID/UID main buffer, bounded cursor                                                                                                                                    | Native 1tJWXL full input/rotation/install regression; layout ErnuOt14; partial-transfer disconnect wYdLH1; actual model SOE0wa | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_setup_plan`    | 06 / P4/P5.5    | AndroidSetupPlanInput/AndroidSetupView; android.read + setup (inventory also manage); bounded inventory/catalog and immutable native plan; protocol/android_setup.rs, broker/android_setup.rs and android/agent_setup.rs                | Native23 7dOf1P, core2/full broker61, Android units75, WebKit UI2 and wire5                                                    | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_setup_apply`   | 06 / P4/P5.5    | AndroidSetupApplyInput/AndroidManagementResult; android.setup/read; exact plan revision, Settings provider consent, durable native receipt; protocol/android_setup.rs, broker/android_setup.rs and android/agent_setup.rs               | Native23 7dOf1P, core2/full broker61, Android units75, WebKit UI2 and wire5                                                    | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_android_device_manage` | 06 / P4/P5.5    | AndroidDeviceManageInput; android.manage/read or global setup/read; exact device/metadata identity, protected Settings confirmation and native installer; protocol/android_setup.rs, broker/android_setup.rs and android/agent_setup.rs | Native23 7dOf1P, core2/full broker61, Android units75, WebKit UI2 and wire5                                                    | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_project_open`          | 07 / P5.3       | ProjectOpenInput/ProjectOpened; project.open + workspace.write/panel.create; Settings root approval; project_open.rs                                                                                                                    | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_project_close`         | 07 / P5.3       | ProjectCloseInput/ProjectClosure; project.close + all workspace grants + ancestor scopes; broker/project_close.rs and shared bridge                                                                                                     | Native 1tJWXL; Android ErnuOt14, Chat 0ovWOt54, project-close wGLMu7; broker68 and retained-layout UI/model tests              | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_editor_open`           | 07 / P5.1       | EditorOpenInput/EditorOpened or EditorPreviewed; files.read + editor.read + panel.create/focus; shared runtime                                                                                                                          | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_editor_read`           | 07 / P5.1       | EditorReadInput/EditorText; editor.read + files.read; shared buffer and canonical source proof                                                                                                                                          | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_editor_apply_edits`    | 07 / P5.1       | EditorEditsInput/EditorEdited; editor.write/read + files.read; one revision-bound UTF-16 undo transaction                                                                                                                               | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_editor_save`           | 07 / P5.1       | EditorSaveInput/EditorSaved; files.mutate/read + editor.write/read; pinned atomic save and native receipt                                                                                                                               | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_files_list`            | 07 / P5.1       | FilesListInput; files.read; descriptor-relative directory, bounded revision-bound pages                                                                                                                                                 | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_files_read`            | 07 / P5.1       | FilesReadInput/FileText; files.read; pinned file, bounded decoded disk text and exact revision                                                                                                                                          | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_files_search`          | 07 / P5.1       | FilesSearchInput; files.read; bounded directory traversal, secret policy and owner-scoped cursors                                                                                                                                       | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_files_mutate`          | 07 / P5.1       | FilesMutateInput/FilesMutated; files.mutate/read plus create/rename/trash; native atomic writes and shared aliases                                                                                                                      | Native 1tJWXL and previews 88IOPg; core/broker and shared-buffer UI; actual APFS ENOSPC; rename hdyUBf/Nzkg07/LBLkKN           | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_git_status`            | 07 / P5.2       | GitStatusInput; git.read + files.read; pinned repository, redacted bounded revision-bound status                                                                                                                                        | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_git_diff`              | 07 / P5.2       | GitDiffInput; git.read + files.read; explicit repository/path/comparison and bounded revision                                                                                                                                           | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_git_history`           | 07 / P5.2       | GitHistoryInput; git.read + files.read; pinned start commit and bounded history pages                                                                                                                                                   | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_git_mutate`            | 07 / P5.2       | GitMutateInput/GitMutated; seven typed operations, operation scopes and exact UI/native approval; git_mutate.rs                                                                                                                         | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_settings_read`         | 07 / P5.4       | SettingsReadInput/SettingsSnapshot, global settings.read, broker/settings_read.rs and retained provider bridge                                                                                                                          | Native KzW0fU/qqYreQ/0GkHVO/u6PGm8/GJqZd0; protected writes, conflicts, recovery and normal cleanup                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_settings_update`       | 07 / P5.4       | SettingsUpdateInput/SettingsUpdated; settings.read+write, exact Settings decision; closed editor/terminal/shortcut patches                                                                                                              | Native KzW0fU/qqYreQ/0GkHVO/u6PGm8/GJqZd0; protected writes, conflicts, recovery and normal cleanup                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_settings_open`         | 07 / P5.4       | SettingsOpenInput/SettingsOpened; settings.open; broker/settings.rs, native settings_window shared request                                                                                                                              | Native KzW0fU/qqYreQ/0GkHVO/u6PGm8/GJqZd0; protected writes, conflicts, recovery and normal cleanup                            | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_plugins_list`          | 07 / P5.4       | Not exposed; explicitly excluded by the user                                                                                                                                                                                            | Selected scope override (2026-09-23)                                                                                           | NOT_SELECTED                                         |
| `lomi_plugin_open`           | 07 / P5.4       | Not exposed; explicitly excluded by the user                                                                                                                                                                                            | Selected scope override (2026-09-23)                                                                                           | NOT_SELECTED                                         |
| `lomi_chat_list`             | 07 / P5.6       | chat.read + exact Settings-selected IDs and project binding; at most 64 metadata records                                                                                                                                                | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_chat_open`             | 07 / P5.6       | ChatOpenInput/ChatOpened; chat.read/open + panel.create/focus, new additionally chat.create                                                                                                                                             | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_chat_draft`            | 07 / P5.6       | ChatDraftInput/ChatDraftUpdated; chat.read/draft + exact open panel and selected conversation                                                                                                                                           | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_chat_read`             | 07 / P5.6       | chat.read + exact selected conversation; persisted draft or message text                                                                                                                                                                | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_chat_send`             | 07 / P5.6       | chat.read/send, selected exact target, configured connection/model, three revisions and durable request IDs                                                                                                                             | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_chat_stop`             | 07 / P5.6       | chat.read/stop, exact selected conversation/project and request ID; durable retry                                                                                                                                                       | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_chat_export`           | 07 / P5.6       | chat.read/export, exact selected conversation/project; Markdown/JSON text pages, no file writes                                                                                                                                         | Native 0ovWOt54 / iyLq7U54, all seven tools and shared layout; local fixture provider only, no paid requests                   | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_app_prepare_close`     | 07 / P5.7       | Not exposed; explicitly excluded by the user                                                                                                                                                                                            | Selected scope override (2026-09-23)                                                                                           | NOT_SELECTED                                         |
| `lomi_git_commit`            | 07 / P5.2       | GitCommitInput; git.read + files.read; bounded full commit details                                                                                                                                                                      | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_git_remotes`           | 07 / P5.2       | GitRemotesInput; git.read + files.read; credential-redacted remote metadata                                                                                                                                                             | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |
| `lomi_git_open`              | 07 / P5.2       | GitOpenInput/GitOpened; git.read + files.read; explicit diff/commit shared views; panel.create/focus                                                                                                                                    | Native 1tJWXL and 88IOPg: reads/views, seven mutations, hooks and conflicts; broker68; model git-review sivenL                 | VERIFIED — selected macOS ARM64 adapter; P6 separate |

Chapter 07 scope: artifact export is implemented and verified above. Further
theme/plugin management and application update/restart preparation are
NOT_SELECTED by the user's explicit scope change. No generic dispatch exists.

## Contract implementation history

The following dated increments preserve the design and then-current gaps.
Their TODO/IN_PROGRESS statements are historical; the current catalog and
later executed evidence above supersede those statements for the selected host.
