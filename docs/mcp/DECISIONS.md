...
- D14: Agent terminal creation uses native-generated panel/runtime IDs and a one-use spawn ticket. The immutable `/bin/zsh` profile revision is captured in the grant and rechecked at native spawn. Workbench creates the actual tab and the retained xterm runtime owns its existing binary Channel. Only managed PTYs use nonblocking I/O; ordinary terminals retain their previous behavior.
